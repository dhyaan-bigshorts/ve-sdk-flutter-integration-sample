...
class MainActivity : FlutterActivity() {
    ...
    companion object {
        ...
        /**
         * true - uses custom audio browser implementation in this sample false - to keep default
         * implementation
         */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        MethodChannel(appFlutterEngine.dartExecutor.binaryMessenger, "banubaSdkChannel")
                    ...
                    when (call.method) {
                        METHOD_INIT_VIDEO_EDITOR -> {
                            ...
                            if (videoEditorSDK == null) {
                                ...
                                if (videoEditorModule == null) {
                                    ...
                                    videoEditorModule =
                                            VideoEditorModule().apply { initialize(application) }
                                }
                                result.success(null)
                            }
                        }
                            ...
                            checkSdkLicenseVideoEditor(
                                    callback = { isValid ->
                                        if (isValid) {
                                            ...
                                            // ❌ Use of SDK is restricted: the license is revoked or
                                            // expired
                                ...
                                checkSdkLicenseVideoEditor(
                                        callback = { isValid ->
                                            if (isValid) {
                                                ...
                                                // ❌ Use of SDK is restricted: the license is
                                                // revoked or expired
                                                result.error(ERR_CODE_SDK_LICENSE_REVOKED, "", null)
                                            }
                                        },
                        ...
                        METHOD_RELEASE_VIDEO_EDITOR -> {
                            ...
                            if (videoEditorModule != null) {
                                val utilityManager =
                                        try {
                                            // EditorUtilityManager is NULL when the token is
                                            // expired or revoked.
                                            ...
                                            Log.w(
                                                    TAG,
                                                    "EditorUtilityManager was not initialized!",
                                                    e
                                            )
                                            result.error(
                                                    "EditorUtilityManager was not initialized!",
    ...
    private fun startVideoEditorModeNormal() {
        startActivityForResult(
                VideoCreationActivity.startFromCamera(
                        ...
                        // set TrackData object if you open VideoCreationActivity with preselected
                        // music track
                        audioTrackData = null,
    ...
    private fun startVideoEditorModePIP(pipVideo: Uri) {
        startActivityForResult(
                VideoCreationActivity.startFromCamera(
                        ...
                        // set PiP video configuration
                        pictureInPictureConfig =
                                PipConfig(video = pipVideo, openPipSettings = false)
                ),
                VIDEO_EDITOR_REQUEST_CODE
        )
    }
    ...
    private fun preparePhotoExportData(result: Intent?): Map<String, Any?> {
        ...
        val data = mapOf(ARG_EXPORTED_PHOTO_FILE to photoUri?.toString())
        return data
    }

    private fun checkSdkLicenseVideoEditor(callback: LicenseStateCallback, onError: () -> Unit) {
